...
define check_host_rpath
	$(if $(filter install-host,$(2)),\
		$(if $(filter end,$(1)),${TOOLSDIR}/support/scripts/check-host-rpath $(3) $(HOST_DIR) $(PER_PACKAGE_DIR)))
endef
...
define check_bin_arch
	${TOOLSDIR}/support/scripts/check-bin-arch -p $($(PKG)_NAME) \
...
define pkg-graph-depends
	...
	@cd "$$(CONFIG_DIR)"; \
	$$(TOOLSDIR)/support/scripts/graph-depends $$(BR2_GRAPH_DEPS_OPTS) \
...
# patches, as they are handled specially afterwards.
	$$(foreach e,$$($(2)_ACTUAL_SOURCE_TARBALL) $$(notdir $$($(2)_EXTRA_DOWNLOADS)),\
		$$(TOOLSDIR)/support/scripts/hardlink-or-copy \
			$$($(2)_DL_DIR)/$$(e) \
